              ...
              {filtrados.map(f => {
                ...
                return (
                  <tr key={f.id} className="border-t border-slate-800 hover:bg-sky-500/5">
                    <td className="px-3 py-2.5"><p className="font-semibold text-slate-100">{f.nome}</p><p className="text-slate-500 text-xs font-mono">{f.codigo}</p></td>
                    <td className="px-3 py-2.5 text-slate-300 text-xs">{TIPO_LABEL[f.tipo] || f.tipo}</td>
                    <td className="px-3 py-2.5 font-mono text-slate-400 text-xs">{f.cnpj || '—'}</td>
                    <td className="px-3 py-2.5 text-slate-300">{f.responsavel || '—'}</td>
                    <td className="px-3 py-2.5 text-center">{f.camaraFria ? <Snowflake className="h-4 w-4 text-cyan-400 mx-auto" /> : <span className="text-slate-600">—</span>}</td>
...
function ModalFilial({ item, onClose, onSalvo }: { item: any | null; onClose: () => void; onSalvo: () => void }) {
  ...
  return (
    <Modal titulo={item ? 'Editar Box / Filial' : 'Novo Box / Filial'} onClose={onClose} onSalvar={salvar} salvando={salvando} salvarLabel={item ? 'Salvar' : 'Cadastrar'}>
      ...
      <Secao icon={<User className="h-3.5 w-3.5" />} titulo="Operação & armazenagem" />
      <div className="grid grid-cols-3 gap-3">
